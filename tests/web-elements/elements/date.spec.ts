...
async function selectTimepicker(label: string, hour: string, minute: string, second: string, page: Page) {
    ...
    let okButton = `//div[contains(concat(' ', @class, ' '), ' ant-picker-footer ')]//button[normalize-space()='OK']`;
...
async function selectDateByLabel(label: string, day: string, month: string, year: string, page: Page) {
    ...
    if (year < minYear) {
        ...
        // let backButtonXpath = `//button[@aria-label = 'Last year (Control + left)']`;
    ...
    if (year > maxYear) {
        ...
        // let clickCount = Math.ceil(distance / decade);
        // let nextButtonXpath = `//button[@aria-label = 'Next year (Control + right)']`;
        // for (let i = 0; i < clickCount; i++) {
    ...
    // let yearXpath = `//td[contains(concat(' ', @class, ' '), ' ant-picker-cell-in-view ') and normalize-space() ='${year}']`;
    // await page.locator(yearXpath).click();
    // let monthXpath = `//td[contains(concat(' ', @class, ' '), ' ant-picker-cell-in-view ') and normalize-space() ='${month}']`;
    // await page.locator(monthXpath).click();
    // let dayXpath = `//td[contains(concat(' ', @class, ' '), ' ant-picker-cell-in-view ') and normalize-space() ='${day}']`;
    // await page.locator(dayXpath).click();
}

async function findDecadeByLabel(label: string, distance: number, decade: number, page: Page) {
    let clickCount = Math.ceil(distance / decade);
    let nextButtonXpath = `//button[@aria-label = '${label}']`;
    for (let i = 0; i < clickCount; i++) {
        await page.locator(nextButtonXpath).click();
    }
}
